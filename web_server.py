    ...
    def do_POST(self):
        ...
        for key in sorted(request):
            ...
            if i <= limit - 1:
                ...
                self.wfile.write(
                    bytes("<input type=\"hidden\" name=\"%s\" value=\"%s\">" % (forms[i], offset_for_citations[i]),
                          encoding='utf-8'))

                # The same is done for limit.  
                self.wfile.write(bytes("<p>Limit for citations: <input type=\"number\""
                                       "name=\"%s\" value=\"%s\"></p><ul>" % (forms2[i], limit_for_citations[i] - 1),
                                       encoding='utf-8'))

                # Then citations are printed.
                # If there are limit citations in the dictionary or less, then print all them.
                if len(request[key]) <= limit_for_citations[i] - 1:
                    for value in request[key]:
                        self.wfile.write(bytes("<li>%s</li>" % value, encoding='utf-8'))

                # Otherwise print all except the last one because before we had asked for one more than a user needed.
                else:
                ...
                if offset_for_citations[i] == 0 and len(request[key]) > limit_for_citations[i] - 1:
                    self.wfile.write(
                        bytes("</ul></li><p><input type=\"submit\" name =\"%s\" value =\"Next\"></p>" % next_buttons[i],
                              encoding='utf-8'))
                ...
                # If it's not the first page and we can go further, then all buttons are created. 
                    ...
                    self.wfile.write(bytes("</ul></li><p><input type=\"submit\" name =\"%s\" value =\"Back\"> "
                                           ...
                                           "<input type=\"submit\" name =\"%s\" value =\"At the start\"></p>" % (
                                               back_buttons[i], next_buttons[i], start_buttons[i]), encoding='utf-8'))
                ...
                # If it's the last page, print only 'back' and 'start'.
                    ...
                    self.wfile.write(bytes("</ul></li><p><input type=\"submit\" name =\"%s\" value =\"Back\"> "
                                           "<input type=\"submit\" name =\"%s\" value =\"At the start\"></p>" % (
                                               back_buttons[i], start_buttons[i]), encoding='utf-8'))
        ...
        if offset == 0 and len(request.keys()) > limit:
            ...
            self.wfile.write(bytes(
                "</ol><p><input type=\"submit\" name =\"back\" value =\"Back\"> <input type=\"submit\" name =\"next\" \
                value =\"Next\"> "
                "<input type=\"submit\" name =\"start\" value =\"At the start\"></p></form></body></html>",
                encoding='utf-8'))
